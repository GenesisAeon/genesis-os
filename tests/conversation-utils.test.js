...
try {
  ...
  execSync(`node scripts/summarize-conversations.js ${fragPath}`, { stdio: 'inherit' });
  if (!fs.existsSync(path.join('docs', 'sigils', 'conversations-filter-Mandala.json'))) {
    throw new Error('filter output missing');
  }
  if (!fs.existsSync(path.join('docs', 'sigils', 'conversations-summary.json'))) {
    throw new Error('summary output missing');
  }
